...
app.use((ctx, next) => {
  ...
  if (payload.length == 0) {
    ...
    ctx.body = `<p><input onkeyup="if (event.keyCode == 13) window.location.href = '${url}' + event.target.value" maxlength="${MAX_CHARS}" placeholder="${DEFAULT_PAYLOAD}" type="text" /></p><p>Download the latest AuReiNand with customized payload path!</p><p>Check out #3dshacks on Rizon for more information.</p><p>Binary from <a href="http://astronautlevel2.github.io/AuReiNand/">http://astronautlevel2.github.io/AuReiNand/</a></p>`;
    return;
  }
